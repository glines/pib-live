...
import pib.buildtree
import pib.gitmenu
import pib.gitrepobrowser
import pib.menuconfig
import pib.piglittestbrowser
import pib.topmenu
...
def recommended_piglit_tests():
    # XXX
#    mc = pib.menuconfig.MenuconfigDialog(
#        pib.gitmenu.GitRepoMenuItem(
#            os.path.expanduser('~/repos/mesa')))
    mc = pib.menuconfig.MenuconfigDialog(
        pib.topmenu.PiabTopMenu())
    main.original_widget = mc

def select_piglit_tests():
    test_browser = pib.piglittestbrowser.PiglitTestBrowser()
